...
async function init() {
  ...
  document.addEventListener('click', (e) => {
    // Блокируем закрытие меню, если оно только что было открыто долгим нажатием
    ...
    if (!e.target.closest('.preset-dropdown') && !e.target.closest('.preset-menu') && !e.target.closest('.preset-btn') && !e.target.closest('.stream-menu') && !e.target.closest('#dialSwitchStreamBtn')) {
      closePresetMenu();
    }
  ...
  if (dFB) {
    ...
    const startPress = () => {
      ...
      longPressTimer = setTimeout(() => {
        ...
        render();
        showToast(state.dialView === 'full' ? 'Полная шкала' : 'Эффективная шкала');
      }, 500);
    };
    const cancelPress = () => { if (longPressTimer) { clearTimeout(longPressTimer); longPressTimer = null; } };
    
    dFB.addEventListener('mousedown', startPress);
    dFB.addEventListener('mouseup', cancelPress);
    dFB.addEventListener('mouseleave', cancelPress);
    dFB.addEventListener('touchstart', startPress, { passive: true });
    dFB.addEventListener('touchend', cancelPress);
  ...
  if (dCB) {
    dCB.addEventListener('click', () => {
     ...
     // Кнопка смены потока на шкале (левый клик + долгий тап + правый клик)
  const dSSB = document.getElementById('dialSwitchStreamBtn');
  if (dSSB) {
    let pressTimer = null;
    let isLongPress = false;
    
    const startPress = (e) => {
      if (e.button !== 0) return; // Только левая кнопка запускает таймер
      isLongPress = false;
    ...
    dSSB.addEventListener('mousedown', startPress);
    dSSB.addEventListener('mouseup', endPress);
    dSSB.addEventListener('mouseleave', endPress);
    dSSB.addEventListener('touchstart', startPress, { passive: true });
    dSSB.addEventListener('touchend', endPress);
    
    // Обработка отпускания левой кнопки (если не было долгого нажатия)
    dSSB.addEventListener('click', (e) => {
      e.stopPropagation(); // Глобальный клик не должен закрывать только что открытое меню
      
      if (isLongPress) {
        isLongPress = false;
        return; // Меню только что открылось долгим нажатием, игнорируем клик
      }
      ...
      // Обычный короткий клик - меняем поток
      document.getElementById('playerStreamInfo').click();
    });
    
    // Правый клик (ПКМ) сразу открывает меню, блокируя браузерное
    dSSB.addEventListener('contextmenu', (e) => {
      e.preventDefault();
      openStreamMenu(dSSB);
    });
  }
  
    // Кнопка переключения режима перемотки
    const dSMB = document.getElementById('dialSkipModeBtn');
    if (dSMB) {
      dSMB.addEventListener('click', (e) => {
        e.stopPropagation();
        state.skipMode = state.skipMode === 'presets' ? 'freq' : 'presets';
        commitState();
        render();
    ...
    if (dMC) {
      dMC.addEventListener('click', () => {
        if (dMC.title) showToast(dMC.title);
      });
    }
  ...
  document.getElementById('playerPrevBtn').addEventListener('click', () => smartSkip(-1));
  document.getElementById('playerNextBtn').addEventListener('click', () => smartSkip(1));
  ...
  if ('mediaSession' in navigator) {
    ...
    navigator.mediaSession.setActionHandler('previoustrack', () => smartSkip(-1));
    navigator.mediaSession.setActionHandler('nexttrack', () => smartSkip(1));
    ...
    try { navigator.mediaSession.setActionHandler('seekbackward', () => smartSkip(-1)); } catch (e) {}
    try { navigator.mediaSession.setActionHandler('seekforward', () => smartSkip(1)); } catch (e) {}
...
function applyViewMode() {
  ...
  // Интегрирована логика applySettingsMode
  const showStatus = state.viewMode === 'setup' || state.viewMode === 'player';
  const display = state.viewMode === 'setup' ? 'block' : 'none';
