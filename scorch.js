...
(() => {
    ...
    const START_TANKS = 5;
    const TANK_HP = 100;
    const NCOL = () => Math.max(300, Math.round(Wc / 1.5)); // 1.5px columns: smooth slopes
  
    const ARSENAL = [
      { key: 'MISSILE',  name: 'Ракета',      r: 30, type: 'blast',   ammo: Infinity, col: '#d8d8d8', dmg: 40, wind: 0.35, shape: 'rocket' },
      { key: 'NUKE',     name: 'Ядерный',     r: 70, type: 'blast',   ammo: 1, col: '#e8c14a', style: 'nuke', dmg: 100, wind: 0.15, shape: 'bomb', power: 2.1 },
      { key: 'BABYNUKE', name: 'Мини-ядер',   r: 48, type: 'blast',   ammo: 3, col: '#e0c060', style: 'nuke', dmg: 85, wind: 0.15, shape: 'bomb', power: 1.4 },
      { key: 'MIRV',     name: 'МИРВ',        r: 34, type: 'mirv',    ammo: 2, col: '#c05a4a', subs: 5, dmg: 35, wind: 0.25, shape: 'mirv' },
      { key: 'FUNKY',    name: 'Фанки-бомба', r: 44, type: 'funky',   ammo: 2, col: '#9a8ac8', bursts: 6, dmg: 30, wind: 0.3, shape: 'cluster' },
      { key: 'ROLLER',   name: 'Роллер',      r: 32, type: 'roller',  ammo: 4, col: '#5aa8a0', dmg: 50, wind: 0.05, shape: 'ball' },
      { key: 'NAPALM',   name: 'Напалм',      r: 50, type: 'napalm',  ammo: 3, col: '#d85a18', dmg: 7, wind: 0.6, shape: 'canister' },
      { key: 'DIGGER',   name: 'Копатель',    r: 72, type: 'digger',  ammo: 4, col: '#8a6a3a', dmg: 0, wind: 0.2, shape: 'drill' },
      { key: 'TUNNEL',   name: 'Бур',         r: 80, type: 'tunnel',  ammo: 3, col: '#a8a8a8', dmg: 0, wind: 0.1, shape: 'drill' },
      { key: 'SAND',     name: 'Песочная',    r: 76, type: 'sand',    ammo: 5, col: '#cbb490', dmg: 0, wind: 0.3, shape: 'ball' },
      { key: 'LIQUID',   name: 'Жидкая грязь',r: 84, type: 'liquid',  ammo: 5, col: '#6a5a40', dmg: 0, wind: 0.3, shape: 'ball' }
    ];
    const TERRAIN_WEAPONS = ['digger', 'sand', 'liquid', 'tunnel'];
    ...
    let cols, waterLevel, biome, tod, seed, S, noise, archetype;
    let tanks, wind, windDir, aiSkill;
    let ammoInv = {}, cur = 0, turn, state;
    ...
    let score = 0, shots = 0, roundStart = 0;
    let playerTanks = START_TANKS, aiTanks = START_TANKS;
    let wins = 0, totalRounds = 0;
    let drag = null, killed = null, helpOpen = false, lastHitInfo = null;
    let lastKillMethod = 'weapon', lastShotApex = 0, waveEnergy = 0;
    let AC = null;
    ...
    function saveRec() {
      const recs = records();
      recs.push({ score, wins, rounds: totalRounds, date: new Date().toLocaleString('ru-RU') });
    ...
    function apocalypsis() {
      ...
      tanks.forEach((t, i) => {
        boomsAt(t.x, t.y - 10, 60, 'nuke', 2.1, 0);
        t.dead = true; killed = i;
        remains.push({ x: t.x, y: t.y, col: t.col, style: 'nuke', falling: true, sunk: false });
        tankParts(t);
      });
      for (let i = 0; i < 4; i++) {
        const bx = R(Wc * 0.15, Wc * 0.85);
        schedule(() => boomsAt(bx, surfaceAt(bx) - 10, 80, 'nuke', 1.8, 0), 0.12 + i * 0.14);
      }
      let fr = 0;
      const fin = () => {
        fr++;
    ...
    function genTerrain() {
      ...
      biome = BIOMES[biomeKeys[Math.floor(S() * biomeKeys.length)]];
      const todKeys = Object.keys(TOD);
      tod = TOD[todKeys[Math.floor(S() * todKeys.length)]];
    ...
    const WP = {
      ...
      beatSense: 1.45, beatCooldown: 0.16, maxRipples: 16, specular: 0.55, fresnel: 0.5
    };
    let ripples = [];
    let waterH = null;
    ...
    function readAudio() {
      ...
      if (!audioLive) {
        aState.bass *= 0.8; aState.mid *= 0.8; aState.treble *= 0.8;
        return;
      }
      ...
      try {
        ...
        // beat: adaptive threshold — follows the track's own dynamics
    ...
    function stepWater(dt) {
      ...
      // ambient: alive ONLY with sound; paused = frozen glass
      const playing = audioLive && (aState.bass + aState.mid + aState.treble > 0.02);
      const k = playing ? 1 : 0;
      if (playing) {
        const bAmp = aState.bass * 6.5;
        const mAmp = aState.mid * 3.5;
        const tAmp = aState.treble * 1.8;
        ...
        // interference: sum fronts into the field
        ripples.forEach(rp => {
        ...
        // decay residual toward flat glass
    ...
    function musicEnergy() {
      return clamp(aState.bass * 0.5 + aState.mid * 0.35 + aState.treble * 0.15, 0, 1);
    }
    ...
    function placeTanks() {
      ...
      if (spots.length < 2) {
        // raise two hilltops above water — guaranteed pads on islands
        const dryIdx = [];
      ...
      // widest-gap pairing over random tries
      ...
      tanks = [
        { x: pi * cols.step, hp: TANK_HP, col: '#2ecc71', dead: false, fallFrom: undefined },
        { x: ei * cols.step, hp: TANK_HP, col: '#ff4757', dead: false, fallFrom: undefined }
      ];
    ...
    function newRound(first) {
      ...
      roundStart = Date.now();
      if (!first) totalRounds++;
      draw();
    }
  
    // ================= DEFORMATION =================
    function craterMask(cx, r, pow, mode) {
      pow = pow || 1;
      const N = cols.length;
      const i0 = clamp(Math.round((cx - r * 1.1) / cols.step), 0, N - 1);
      const i1 = clamp(Math.round((cx + r * 1.1) / cols.step), 0, N - 1);
      const list = [];
      for (let i = i0; i <= i1; i++) {
        ...
        let to = null;
        if (mode === 'add') {
          if (Math.abs(dx) < 1) to = cols[i].top - r * 0.75 * Math.sqrt(Math.max(0, 1 - dx * dx)) * j;
        } else if (mode === 'smooth') {
          const prev = cols[clamp(i - 1, 0, N - 1)].top, next = cols[clamp(i + 1, 0, N - 1)].top;
          to = (cols[i].top * 2 + prev + next) / 4;
        } else {
          if (Math.abs(dx) < 0.74) to = cols[i].top + r * M().depthF * pow * Math.pow(1 - dx * dx, 0.75) * j;
          else if (Math.abs(dx) < 1.06) {
            const f = 1 - (Math.abs(dx) - 0.74) / 0.32;
            to = cols[i].top - r * M().rimF * pow * Math.pow(f, 1.2) * j;
          }
        }
    ...
    // ================= FX =================
    function boomsAt(x, y, r, style, pow, dmg) {
      pow = pow || 1;
      dmg = dmg || 0;
      const m = M();
      ...
      schedule(() => craterMask(x, r * (nuke ? 1.15 : 1), pow), 0.12);
      schedule(() => spawnChunks(x, y, r, m.chunkN * pow), 0.16);
      ...
      if (dmg > 0) tanks.forEach((tk, i) => {
        if (!tk.dead && Math.hypot(tk.x - x, tk.y - 6 - y) < r * 1.6 * pow) damageTank(i, dmg, style, x, y);
      });
    }
    ...
    function spawnSandRain(x, r, col) {
      for (let i = 0; i < Math.min(46, r); i++) {
    ...
    function stepFx(dt) {
      ...
      remains.forEach(rm => {
        if (!rm.falling || rm.sunk) return;
        const gy = surfaceAt(rm.x);
        const wy = waterAt(rm.x);
        if (rm.y < gy - 1 && gy < wy) {
          rm.y += 300 * dt;
          if (rm.y >= gy) { rm.y = gy; rm.falling = false; }
        } else if (rm.y < wy - 6 && gy > wy) {
          rm.y += 300 * dt;
          if (rm.y >= wy - 6) {
            rm.sunk = true; rm.falling = false;
            fx.push({ k: 'splash', x: rm.x, y: waterAt(rm.x), r: 12, t: 0, life: 0.5 });
    ...
    function fire() {
      ...
      const w = ARSENAL[cur];
      if (ammoInv[w.key] <= 0) { cur = 0; draw(); return; }
    ...
    function aiPickWeapon() {
      const r = Math.random();
      if (tanks[0].hp <= 30 && r < 0.5) return ARSENAL[0];
      if (r < 0.35) return ARSENAL[0];
      if (r < 0.5) return ARSENAL[5];
      if (r < 0.65) return ARSENAL[4];
      if (r < 0.8) return ARSENAL[2];
      if (r < 0.9) return ARSENAL[6];
      return ARSENAL[1];
    }
    function aiTurn() {
    ...
    function updateProjectile(p, dt) {
      ...
      if (p.w.type === 'mirv' && !p.split && p.vy >= 0 && gt - p.t0 > 0.5) {
        ...
        for (let i = 0; i < p.w.subs; i++) {
          subshots.push({
            ...
            w: { ...p.w, type: 'blast', r: p.w.r * 0.75, style: 'plain', wind: p.w.wind, power: 0.7 },
      ...
      if (p.x >= 0 && p.x <= Wc) {
        if (!isTerr(p.w.type) && p.w.type !== 'napalm') {
          tanks.forEach((tk, i) => {
            if (!tk.dead && Math.abs(p.x - tk.x) < 14 && p.y > tk.y - 24 && p.y < tk.y + 8) {
              p.dead = true; damageTank(i, p.w.dmg, p.w.style || 'plain', p.x, p.y);
            }
          });
          if (p.dead) return;
        }
        ...
        if (p.w.type === 'roller' && p.y >= surf - 6 && p.y < surf + 16) {
          ...
          tanks.forEach((tk, i) => { if (!tk.dead && Math.abs(p.x - tk.x) < 13) { p.dead = true; killed = i; } });
          return;
        }
    ...
    function resolveHit(p) {
      ...
      switch (w.type) {
        case 'blast': boomsAt(x, y, w.r, w.style || 'plain', w.power || 1, w.dmg); break;
        case 'mirv': break;
        case 'funky': funkyBoom(x, y, w); break;
        case 'roller': boomsAt(x, y, w.r, 'plain', 1, w.dmg); break;
        case 'napalm': napalmBoom(x, y, w); break;
        case 'digger':
          fx.push({ k: 'shock', x, y, r0: 6, r1: w.r, t: 0, life: 0.25 });
          spawnDust(x, y, w.r * 0.5, M().dustN * 0.7);
          digCorridor(x, y, ang, w.r * 1.6, w.r * 0.6);
          sfx(0.4);
          break;
        case 'tunnel':
          fx.push({ k: 'shock', x, y, r0: 6, r1: w.r * 0.8, t: 0, life: 0.3 });
          spawnDust(x, y, w.r * 0.5, M().dustN);
          digCorridor(x, y, ang, w.r * 2.4, w.r * 0.5);
          sfx(0.5);
          break;
        case 'sand':
          craterMask(x, w.r, 1, 'add');
          spawnSandRain(x, w.r);
          fx.push({ k: 'dustc', x, y: surfaceAt(x) - w.r * 0.6, r: w.r * 0.5, t: 0, life: 1.2, col: M().dustCol });
          sfx(0.35);
          break;
        case 'liquid': {
          craterMask(x, w.r, 1, 'smooth');
          craterMask(x, w.r * 0.45, 1, 'add');
          const i0 = clamp(Math.round((x - w.r * 0.8) / cols.step), 0, cols.length - 1);
          const i1 = clamp(Math.round((x + w.r * 0.8) / cols.step), 0, cols.length - 1);
          let s = 0, n = 0;
          for (let i = i0; i <= i1; i++) { s += cols[i].top; n++; }
          const avg = s / n;
          for (let i = i0; i <= i1; i++) if (cols[i].top > avg + 25) cols[i].top = avg + 25;
          spawnSandRain(x, w.r * 0.6, '#6a5a40');
          sfx(0.3);
          break;
        }
      }
    ...
    function funkyBoom(x, y, w) {
      for (let i = 0; i < w.bursts; i++) {
        schedule(() => boomsAt(x + R(-w.r * 2, w.r * 2), y + R(-w.r * 1.2, w.r * 0.4), w.r * R(0.3, 0.55), 'funky', 0.5, w.dmg), 0.14 * i + R(0, 0.06));
      }
      schedule(() => boomsAt(x, y, w.r, 'plain', 0.9, w.dmg), w.bursts * 0.14 + 0.25);
    }
    function napalmBoom(x, y, w) {
      fx.push({ k: 'flash', x, y, r: w.r * 0.6, t: 0, life: 0.08 });
      for (let i = 0; i < 18; i++) {
        liquids.push({ x: x + R(-w.r / 2, w.r / 2), y, vx: R(-45, 45), vy: R(-100, -25), t: 0, w });
      }
    ...
    function damageTank(i, baseDmg, style, x, y) {
      ...
      const wref = ARSENAL.find(w => w.style === style);
    ...
    function killTank(i, cause, style) {
      ...
      if (cause === 'drown') {
        ...
        fx.push({ k: 'splash', x: t.x, y: waterAt(t.x), r: 12, t: 0, life: 0.5 });
      } else if (cause === 'crush') {
        fx.push({ k: 'dustc', x: t.x, y: t.y - 10, r: 20, t: 0, life: 0.8, col: M().dustCol });
        remains.push({ x: t.x, y: t.y, col: t.col, style: 'sand', falling: false });
      } else {
        boomsAt(t.x, t.y - 10, 34, style === 'nuke' ? 'nuke' : 'plain', 0.8, 0);
        remains.push({ x: t.x, y: t.y, col: t.col, style: style || 'plain', falling: true, sunk: false });
        tankParts(t);
      }
    }
    ...
    function endTurn() { state = 'aim'; if (turn === 2) turn = 0; draw(); }
    ...
    function endRound() {
      ...
      if (won) {
        ...
        if (['MISSILE', 'ROLLER', 'DIGGER', 'TUNNEL', 'SAND', 'LIQUID'].includes(ARSENAL[cur].key)) pts = Math.round(pts * 1.5);
        ...
        score += pts;
        aiTanks--; aiSkill = Math.min(0.95, aiSkill + 0.08); playerTanks++;
        ARSENAL.forEach(w => { if (w.ammo !== Infinity && w.ammo <= 5 && ammoInv[w.key] < w.ammo) ammoInv[w.key]++; });
      } else { playerTanks--; aiSkill = Math.max(0.2, aiSkill - 0.05); }
      if (playerTanks <= 0 || aiTanks <= 0) { schedule(() => showOver(aiTanks <= 0), 1.2); return; }
    ...
    function showOver(won) {
      ...
      saveRec();
      const after = records();
      const myIdx = after.findIndex(r => !before.includes(r));
      $('.sc-over-title').textContent = won ? '🏆 Победа!' : '💥 Поражение';
      $('.sc-over-res').innerHTML = `Очки: <b style="color:var(--accent)">${score}</b> · Побед: <b>${wins}</b> · Раундов: <b>${totalRounds}</b>`;
      $('.sc-rectab').innerHTML = '<tr><th>#</th><th>Очки</th><th>Побед</th><th>Дата</th></tr>' +
        after.slice(0, MAX_REC).map((r, i) => `<tr${i === myIdx ? ' class="me"' : ''}><td>${i + 1}</td><td>${r.score}</td><td>${r.wins || 0}</td><td>${r.date}</td></tr>`).join('');
    ...
    // ================= LOOP =================
    function start() {
      playerTanks = START_TANKS; aiTanks = START_TANKS;
      score = 0; wins = 0; totalRounds = 0; shots = 0; aiSkill = 0.35;
      ammoInv = {}; ARSENAL.forEach(w => { ammoInv[w.key] = w.ammo; }); cur = 0;
      newRound(true);
    ...
    function step(dt) {
      ...
      // comets at night: frequent, varied
      if (!isDayT() && Math.random() < dt * 0.6) {
        comets.push({ x: R(0, Wc * 1.1), y: R(10, Hc * 0.3), vx: -R(120, 320), vy: R(30, 110), t: 0, life: R(0.6, 1.8), sz: R(1, 2.4) });
      }
      comets.forEach(c => { c.x += c.vx * dt; c.y += c.vy * dt; c.t += dt; });
      comets = comets.filter(c => c.t < c.life);
  
      // wind carriers
      const want = Math.round(clamp(Math.abs(wind), 0.3, 4) * 10);
      while (windParts.length < want) windParts.push({ x: R(0, Wc), y: R(20, Hc * 0.9), ph: R(0, 6.28), ph2: R(0, 6.28), spd: R(0.6, 1.4), s: R(1, 2.4), a: R(0.05, 0.14), leaf: Math.random() < 0.3 });
    ...
    function booms0() { return !fx.some(f => f.k === 'fire' || f.k === 'flash' || f.k === 'shock') && terraJobs.length === 0 && liquids.length === 0 && !debris.some(d => !d.settled) && firePatches.length === 0; }
    ...
    function drawSky() {
      ...
      if (tod.stars) {
        for (let i = 0; i < 150; i++) {
          const x = ((i * 137.5 + seed % 97) % Wc + Wc) % Wc;
          const y = ((i * 71.3) % (Hc * 0.5));
          const layer = i % 3;
          const tw = 0.2 + 0.8 * Math.abs(Math.sin(skyT * (0.2 + layer * 0.12) + i * 1.7));
          ctx.globalAlpha = [0.8, 0.5, 0.28][layer] * tw * (tod.stars === 'dim' ? 0.35 : 1);
          ctx.fillStyle = ['#fff', '#cfe0ff', '#ffe8c8'][layer];
          ctx.fillRect(x, y, layer === 0 ? 1.6 : 1, layer === 0 ? 1.6 : 1);
        }
        ctx.globalAlpha = 0.08;
        ctx.fillStyle = '#fff';
        for (let i = 0; i < 130; i++) {
          const t = (i * 61.7) % 100 / 100;
          const x = t * Wc, y = Hc * 0.1 + t * Hc * 0.35 + (noise(i * 3.3) - 0.5) * 60;
          ctx.fillRect(x, y, 1, 1);
        }
        ctx.globalAlpha = 1;
        // moon: slow drift along a low arc
        const mu = (skyT * 0.006) % 1;
        const mx = Wc * (0.15 + mu * 0.7);
        const my = Hc * (0.28 - Math.sin(mu * Math.PI) * 0.16);
        ...
        [[-6, -4, 4], [5, 3, 3], [-2, 7, 2.5]].forEach(c => { ctx.beginPath(); ctx.arc(mx + c[0], my + c[1], c[2], 0, Math.PI * 2); ctx.fill(); });
        // comets: nucleus + long tail
        comets.forEach(c => {
          const p = c.t / c.life;
          const sz = c.sz || 1.5;
          ...
          ctx.beginPath();
          ctx.moveTo(c.x, c.y);
          ctx.lineTo(c.x - c.vx * 0.22, c.y - c.vy * 0.22);
          ctx.stroke();
        ...
        // sun: slow drift, sunset starts lower
        const isSunset = tod.stops === TOD.sunset.stops;
        const su = (skyT * 0.008) % 1;
        const sx = Wc * (0.85 - su * 0.75);
    ...
    function drawTerrain() {
      ...
      // smooth anti-aliased surface edge: two half-alpha strokes offset by half pixel
      const edgePath = () => {
        ctx.beginPath();
    ...
    // Water: translucent gradient per column, lighting from gradient, no hard bands
    function drawWater() {
      if (!waterH) return;
      const dark = !isDayT();
      const N = cols.length;
      ...
      for (let i = 0; i < N; i++) {
        ...
        ctx.globalAlpha = dark ? 0.3 : 0.36; // glassy translucent
      ...
      ctx.strokeStyle = dark ? 'rgba(120,180,240,0.25)' : 'rgba(40,100,190,0.3)';
      ctx.lineWidth = 1;
      ctx.beginPath();
      let started = false;
      for (let i = 0; i < N; i++) {
    ...
    function drawRemains() {
      remains.forEach(rm => {
        ...
        ctx.globalAlpha = rm.sunk ? 0.4 : 0.8;
        px(-10, -8, 20, 6, '#1c1a18');
        px(-12, -3, 24, 4, '#12100e');
        px(-3, -12, 6, 4, '#242220');
        ctx.globalAlpha = 0.2 * Math.abs(Math.sin(skyT * 2 + rm.x));
        ctx.fillStyle = '#555';
        ctx.fillRect(rm.x + 2, rm.y - 20 - Math.sin(skyT * 1.5) * 3, 3, 3);
        ctx.globalAlpha = 1;
      });
    }
    ...
    function drawTanks() {
      tanks.forEach((t, i) => {
        ...
        px(-12, -6, 24, 5, '#333'); px(-12, -6, 24, 1, '#555'); px(-12, -2, 24, 1, '#111');
        for (let k = -10; k <= 10; k += 4) px(k, -6, 2, 5, '#222');
        px(-10, -11, 20, 6, hull); px(-10, -11, 20, 1, shade(t.col, 1.4)); px(-10, -6, 20, 1, shade(hull, 0.6));
        px(-4, -15, 8, 4, shade(hull, 0.8)); px(-4, -15, 8, 1, shade(t.col, 1.2));
        const ang = i === 0 ? aim.ang : aiAim;
        const dir = i === 0 ? playerDir() : (tanks[1].x < tanks[0].x ? 1 : -1);
        ctx.translate(t.x, t.y - 13);
        ctx.rotate((dir === 1 ? -ang : ang - 180) * Math.PI / 180);
        ctx.fillStyle = shade(t.col, 1.1);
        ctx.fillRect(0, -1.5, 18, 3);
        ctx.fillStyle = shade(t.col, 0.8);
        ...
        if (t.hp < 25) {
          ...
          ctx.beginPath(); ctx.moveTo(t.x - 3, t.y - 15); ctx.lineTo(t.x, t.y - 15 - fl); ctx.lineTo(t.x + 3, t.y - 15); ctx.fill();
          ctx.fillStyle = '#ffd23f';
          ctx.beginPath(); ctx.moveTo(t.x - 1.5, t.y - 15); ctx.lineTo(t.x, t.y - 15 - fl * 0.6); ctx.lineTo(t.x + 1.5, t.y - 15); ctx.fill();
        }
      });
    }
    ...
    // Aim preview: same integrator as flight, wind=0 — correction is player's skill.
    function drawAim() {
      const t = tanks[0];
      const dir = playerDir();
    ...
    function drawHUD() {
      ...
      $('.sc-round').textContent = totalRounds + 1;
      $('.sc-wins').textContent = wins;
      $('.sc-score').textContent = score;
      $('.sc-you').textContent = `${biomeLabel()}: ${playerTanks}`;
      $('.sc-enemy').textContent = `Враг: ${aiTanks}`;
    ...
    const biomeLabel = () => ({ green: 'Холмы', desert: 'Пустыня', arctic: 'Арктика', volcanic: 'Вулкан' }[Object.keys(BIOMES).find(k => BIOMES[k] === biome)] || '');
        ...
        <div class="sc-wrap">
          ...
          <div class="sc-help">
            ...
            <div style="color:var(--text-dim);font-size:11px">
              Линия прицела — траектория без ветра: шевроны у вершины показывают ветер, поправка за вами.
              Материал грунта меняет кратер: песок глубоко и осыпается, скала мелко и много обломков.
              Вода топит, навесные удары подрубают опору — танк сползает в кратер. Вода живёт от музыки (CORS-станции).
            </div>
            <h5>Оружие</h5>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:2px 12px;font-size:11px">
              <div>1 Ракета 40hp ∞</div><div>2 Ядерный 100hp 1</div>
              <div>3 Мини-ядер 85hp 3</div><div>4 МИРВ 5×35hp 2</div>
              <div>5 Фанки 6×30hp 2</div><div>6 Роллер 50hp 4</div>
              <div>7 Напалм 7hp/с 3</div><div>8 Копатель — рельеф 4</div>
              <div>9 Бур — тоннель 3</div><div>0 Песок / Грязь 5/5</div>
            </div>
          </div>
          <div class="sc-over">
    ...
    function keyH(e) {
      ...
      if (/^[0-9]$/.test(e.key)) {
        ...
        if (idx < ARSENAL.length && ammoInv[ARSENAL[idx].key] > 0) { cur = idx; overlay.querySelector('.sc-wmenu').classList.remove('show'); draw(); }
        return;
      }
    ...
    function nextWeapon() {
      for (let i = 1; i <= ARSENAL.length; i++) {
        ...
        if (ammoInv[ARSENAL[idx].key] > 0) { cur = idx; draw(); return; }
      }
      cur = 0; draw();
    }
